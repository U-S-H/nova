...
<head>
    ...
    <title>NOVA CORP | MEGA TERMINAL</title>
    ...
    <style>
        ...
        .ticker-wrap { background: #0F172A; color: #D4AF37; padding: 8px 0; font-size: 10px; font-weight: 800; overflow: hidden; }
        .ticker-move { display: inline-block; white-space: nowrap; animation: ticker 25s linear infinite; }
        @keyframes ticker { 0% { transform: translateX(100%); } 100% { transform: translateX(-100%); } }
        .progress-bar { height: 6px; background: #f1f5f9; border-radius: 10px; overflow: hidden; margin-top: 10px; }
...
<body>
    ...
    <div id="auth-screen" class="active-screen min-h-screen bg-white flex flex-col justify-center p-8">
        <div class="text-center mb-10">
            <h1 class="text-6xl font-black italic tracking-tighter">NO<span class="text-gold">VA</span></h1>
            <p class="text-[10px] font-black uppercase text-slate-400 tracking-[0.4em] mt-2">The Ultimate Core</p>
        </div>
        <div class="space-y-4 max-w-xs mx-auto w-full">
            ...
            <input type="text" id="u-ref" placeholder="Ref Code (Optional)" class="w-full p-5 rounded-2xl border bg-slate-50 outline-none font-bold text-xs">
            <button onclick="handleAuth()" class="w-full btn-gold py-5 uppercase text-xs tracking-widest shadow-2xl">Access Terminal</button>
            <p onclick="toggleAuth()" id="auth-btn-text" class="text-[10px] text-center text-slate-400 font-black uppercase cursor-pointer underline">New Here? Create Account</p>
        </div>
    </div>

    <div id="app" class="hidden">
        <div class="ticker-wrap"><div class="ticker-move" id="live-ticker">NOVA CORE: Global servers active • New V21 Nodes deployed • Fast withdrawals enabled • Join the elite network today!</div></div>
        ...
        <header class="p-5 flex justify-between items-center bg-white border-b sticky top-0 z-[100]">
            ...
            <div class="text-right">
                ...
                <p id="b-header" class="text-lg font-black text-slate-950 tracking-tighter">$0.00</p>
            </div>
        </header>
        ...
        <div id="scr-dash" class="screen active-screen px-4 pt-6">
            <div class="bg-slate-950 rounded-[35px] p-8 text-white mb-6 relative shadow-2xl overflow-hidden border-b-4 border-gold">
                <p class="text-[8px] text-slate-500 uppercase font-black mb-2">Portfolio Value</p>
                ...
                <div class="flex justify-between border-t border-white/10 pt-6">
                    ...
                    <div class="text-right"><p class="text-[7px] text-slate-500 uppercase font-black">Ref Income</p><p id="b-team" class="text-xl font-black text-gold">$0.00</p></div>
                </div>
            </div>

            <div class="grid grid-cols-3 gap-3 mb-6 text-center">
                <div onclick="openModal('mod-dep')" class="premium-card p-4"><i class="fa-solid fa-cloud-arrow-up text-gold mb-2"></i><p class="text-[8px] font-black uppercase">Deposit</p></div>
                ...
                <div onclick="claimDaily()" class="premium-card p-4"><i class="fa-solid fa-star text-green-400 mb-2"></i><p class="text-[8px] font-black uppercase">Daily Claim</p></div>
            </div>

            <div class="premium-card p-5 mb-4 flex gap-2">
                <input type="text" id="promo-input" placeholder="Promo Code" class="flex-1 bg-slate-50 p-3 rounded-xl outline-none font-bold text-xs uppercase">
            ...
            <div class="premium-card p-6 border-dashed border-2 border-slate-200">
                <h4 class="text-[9px] font-black text-slate-400 uppercase mb-2">Invite Friends (10% Comm)</h4>
                <div class="flex items-center justify-between bg-slate-50 p-3 rounded-xl">
                    <span id="ref-link" class="text-[10px] font-bold text-slate-500">nova.com/?ref=user</span>
        ...
        <div id="scr-team" class="screen px-4 pt-6">
            <h3 class="font-black text-2xl mb-8 italic uppercase px-2">Your Network</h3>
            <div class="grid grid-cols-2 gap-4 mb-8">
                <div class="premium-card p-6 text-center bg-slate-900 text-white"><h5 id="team-count" class="text-3xl font-black">0</h5><p class="text-[8px] font-black uppercase text-gold">Direct Partners</p></div>
        ...
        <div id="scr-logs" class="screen px-4 pt-6">
            <h3 class="font-black text-2xl mb-8 italic uppercase px-2">Transaction Audit</h3>
            <div id="logs-list" class="space-y-4"></div>
        </div>

        <div id="scr-admin" class="screen px-4 pt-6">
            <h2 class="text-2xl font-black mb-8 text-red-600 uppercase italic px-2">Master Core Control</h2>
            <div class="premium-card p-6 bg-slate-950 text-white mb-6 border-none">
                <p class="text-[8px] uppercase font-black text-slate-500 mb-4 tracking-widest">Gateway Config</p>
                <input type="text" id="adm-ep" placeholder="EasyPaisa Number" class="w-full p-3 rounded-xl text-black mb-2 font-bold">
                <input type="text" id="adm-jc" placeholder="JazzCash Number" class="w-full p-3 rounded-xl text-black mb-4 font-bold">
                <input type="number" id="adm-tax" placeholder="Withdraw Tax (%)" class="w-full p-3 rounded-xl text-black mb-4 font-bold">
                <button onclick="saveAdminSettings()" class="w-full btn-gold py-3 text-[10px] uppercase">Save Settings</button>
            </div>
            <div id="adm-pending" class="space-y-4"></div>
        </div>

        <nav class="nav-bar">
            <div onclick="nav('scr-dash', this)" class="nav-item active"><i class="fa-solid fa-house-user text-xl"></i><span>DASH</span></div>
            <div onclick="nav('scr-nodes', this)" class="nav-item"><i class="fa-solid fa-microchip text-xl"></i><span>NODES</span></div>
            <div onclick="nav('scr-team', this)" class="nav-item"><i class="fa-solid fa-users-viewfinder text-xl"></i><span>TEAM</span></div>
            <div onclick="nav('scr-logs', this)" class="nav-item"><i class="fa-solid fa-file-invoice text-xl"></i><span>LOGS</span></div>
            <div id="nav-adm" onclick="nav('scr-admin', this)" class="nav-item text-red-600" style="display:none;"><i class="fa-solid fa-shield-halved text-xl"></i><span>MASTER</span></div>
        </nav>
    </div>

    <div id="mod-dep" class="fixed inset-0 bg-white z-[2000] p-8 hidden overflow-y-auto">
        <div class="flex justify-between items-center mb-10"><h2 class="font-black text-2xl uppercase italic text-gold">Deposit</h2><i onclick="closeModal('mod-dep')" class="fa-solid fa-xmark text-3xl text-slate-200"></i></div>
        ...
        <div id="way-info" class="bg-slate-950 text-gold p-8 rounded-[35px] mb-8 hidden text-center shadow-2xl border-b-4 border-gold">
            <p id="way-num" class="text-xl font-black tracking-widest"></p>
        </div>
        <input type="number" id="dep-amt" placeholder="Amount ($)" class="w-full p-5 bg-slate-50 rounded-2xl border-none mb-4 font-bold">
        <input type="text" id="dep-tx" placeholder="TRX ID" class="w-full p-5 bg-slate-50 rounded-2xl border-none mb-10 font-bold uppercase">
        <button onclick="handleFinance('Deposit')" class="w-full btn-gold py-6 uppercase tracking-widest">Confirm Deposit</button>
    </div>
    ...
    <script type="module">
        ...
        let sys = { ep: "03279177196", jc: "03279177196", tax: 5 };

        // 21 NODES CONFIG
        const nodes = [];
        for(let i=1; i<=21; i++) {
            nodes.push({ id: i, n: `NOVA-V${i}`, c: 10 + (i * 20), y: 0.5 + (i * 1.5), img: `https://images.unsplash.com/photo-1639322537228-f710d846310a?w=400` });
        }
        ...
        window.handleAuth = async () => {
            ...
            if(!id || !pw) return alert("Fill all credentials!");
            
            const uRef = doc(db, "users", id); const snap = await getDoc(uRef);
            if(mode === 'login') {
                if(snap.exists() && snap.data().pw === pw) {
                    if(snap.data().status === 'Banned') return alert("Account Suspended!");
        ...
        function initApp(id) {
            ...
            onSnapshot(query(collection(db, "users"), where("refBy", "==", id)), s => {
                ...
                s.forEach(doc => {
                    list.innerHTML += `<div class="premium-card p-4 flex justify-between items-center text-[10px]"><b>@${doc.id}</b><span class="text-green-500 font-black">ACTIVE MEMBER</span></div>`;
                });
            });
            ...
            onSnapshot(query(collection(db, "logs"), where("user", "==", id)), s => {
                ...
                s.forEach(d => {
                    ...
                    list.innerHTML += `<div class="premium-card p-5 flex justify-between items-center">
                        ...
                        <div class="text-right"><p class="font-black text-sm">$${l.amount.toFixed(2)}</p><span class="text-[8px] font-black text-gold">${l.status}</span></div>
                    </div>`;
                });
            });
        ...
        function renderNodes(active) {
            ...
            nodes.forEach(n => {
                ...
                grid.innerHTML += `<div class="premium-card">
                    ...
                    ${isRun ? `<div class="px-6 pb-4"><div class="progress-bar"><div class="progress-fill" style="width: 100%; transition: 86400s linear;"></div></div></div>` : ''}
                </div>`;
            });
        }
        ...
        window.claimDaily = async () => {
            ...
            if(Date.now() - (s.data().lastClaim || 0) < 86400000) return alert("Available every 24 hours!");
            await updateDoc(uRef, { bal: s.data().bal + 0.10, lastClaim: Date.now() });
            alert("Success! $0.10 Claimed.");
        };

        window.buyNode = async (id, c, y) => {
            const uRef = doc(db, "users", user); const s = await getDoc(uRef);
            if(s.data().bal < c) return alert("Insufficient Balance");
        ...
        window.handleFinance = async (type) => {
            ...
            if(!amt || !ref) return alert("All fields mandatory");
            await addDoc(collection(db, "logs"), { user, type, amount: amt, ref, status: 'Pending' });
            alert("Ticket Submitted!"); closeModal('mod-dep');
        };

        // ADMIN LOGIC
        document.getElementById('logo-main').onclick = () => {
            tapCount++; if(tapCount >= 5) {
                if(prompt("Enter Terminal Key:") === "nov786") { document.getElementById('nav-adm').style.display="flex"; loadAdm(); }
                tapCount = 0;
            }
        };
                ...
                s.forEach(d => {
                    list.innerHTML += `<div class="premium-card p-5 text-[10px] flex justify-between items-center">
                        ...
                        <div class="flex gap-2">
                            <button onclick="approveLog('${d.id}', '${d.data().user}', ${d.data().amount}, '${d.data().type}')" class="bg-green-500 text-white px-4 py-2 rounded-xl">OK</button>
                            <button onclick="banUser('${d.data().user}')" class="bg-red-600 text-white px-4 py-2 rounded-xl">BAN</button>
                        </div>
                    </div>`;
                });
            });
        ...
        window.approveLog = async (lid, uid, amt, type) => {
            ...
            await updateDoc(doc(db, "logs", lid), { status: 'Approved' }); alert("Action Completed");
        };
        ...
        window.saveAdminSettings = async () => {
            ...
            await setDoc(doc(db, "config", "master"), up, { merge: true }); alert("Platform Settings Updated!");
        };
        ...
        window.showWay = (t) => { document.getElementById('way-info').classList.remove('hidden'); document.getElementById('way-num').innerText = sys[t]; };
        window.openModal = (id) => document.getElementById(id).classList.remove('hidden');
        window.closeModal = (id) => document.getElementById(id).classList.add('hidden');
        window.toggleAuth = () => { mode = mode === 'login' ? 'reg' : 'login'; document.getElementById('auth-btn-text').innerText = mode === 'login' ? "New Here? Create Account" : "Already have account? Login"; };
        window.copyRef = () => { navigator.clipboard.writeText(document.getElementById('ref-link').innerText); alert("Referral Link Copied!"); };
