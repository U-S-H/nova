...
<head>
    ...
    <style>
        ...
        body { background: var(--bg); color: white; font-family: 'Plus Jakarta Sans', sans-serif; -webkit-tap-highlight-color: transparent; }
        ...
        .icon-circle { width: 55px; height: 55px; background: rgba(255, 255, 255, 0.03); border-radius: 18px; display: flex; align-items: center; justify-content: center; font-size: 20px; transition: 0.3s; border: 1px solid rgba(255, 255, 255, 0.05); }
        ...
        .nova-input { background: #0f0f0f; border: 1px solid #1a1a1a; width: 100%; padding: 16px; border-radius: 16px; outline: none; font-size: 14px; }
...
<body>
    ...
    <div id="auth-screen" class="min-h-screen flex flex-col items-center justify-center p-8 active-screen">
        ...
        <div class="app-card p-6 w-full max-w-sm">
            ...
            <p onclick="toggleM()" id="m-text" class="text-center text-[10px] mt-6 text-gray-500 uppercase tracking-widest cursor-pointer">Register New Account</p>
        </div>
    </div>
    ...
    <div id="app" class="hidden">
        <header class="p-6 flex justify-between items-center">
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 app-card flex items-center justify-center text-[#D4AF37] font-black" id="avatar">N</div>
        ...
        <div id="dash-screen" class="screen active-screen p-6 pb-32">
            <div class="app-card p-8 mb-8 gold-glow relative overflow-hidden">
                <div class="absolute -right-5 -top-5 w-24 h-24 bg-[#D4AF37] opacity-5 blur-3xl rounded-full"></div>
                <p class="text-[10px] text-gray-500 font-bold tracking-widest mb-2">PORTFOLIO VALUE</p>
                <h1 id="main-bal" class="text-5xl font-black tracking-tighter mb-4">$0.00</h1>
                <p id="v-daily" class="text-green-500 text-[10px] font-bold">+ $0.00 Daily Profit</p>
            </div>
            ...
            <div class="flex justify-between mb-10 px-2">
                <div onclick="openModal('dep-modal')" class="icon-btn">
                    ...
                    <span class="text-[10px] font-bold text-gray-400">DEPOSIT</span>
                </div>
                <div onclick="openModal('wd-modal')" class="icon-btn">
                    <div class="icon-circle text-red-500"><i class="fa-solid fa-arrow-up-long"></i></div>
                    <span class="text-[10px] font-bold text-gray-400">WITHDRAW</span>
                </div>
                <div onclick="nav('mine-screen')" class="icon-btn">
                    <div class="icon-circle text-yellow-500"><i class="fa-solid fa-microchip"></i></div>
                    <span class="text-[10px] font-bold text-gray-400">MINING</span>
                </div>
                <div onclick="openModal('support-modal')" class="icon-btn">
                    <div class="icon-circle text-blue-500"><i class="fa-solid fa-headset"></i></div>
                    <span class="text-[10px] font-bold text-gray-400">HELP</span>
                </div>
            </div>

            <h3 class="text-[10px] font-black text-gray-600 mb-4 tracking-widest uppercase px-1">Recent Activity</h3>
        ...
        <div id="office-screen" class="screen p-6 pb-32 text-center">
            <div class="app-card p-8 mb-6">
                <i class="fa-solid fa-share-nodes text-3xl text-[#D4AF37] mb-4"></i>
                <p class="text-xs font-bold text-gray-500 mb-2">Referral ID</p>
                <p id="ref-link" class="text-[10px] font-mono text-gray-300 bg-black/50 p-4 rounded-xl border border-white/5"></p>
            </div>
        </div>

        <div id="dep-modal" class="hidden fixed inset-0 bg-black z-[2000] p-6 pt-12 overflow-y-auto">
            <div onclick="closeModal('dep-modal')" class="mb-8 text-gray-500"><i class="fa-solid fa-chevron-left mr-2"></i> BACK</div>
            <h2 class="text-2xl font-black mb-8 gold-text">DEPOSIT ASSETS</h2>
            ...
            <button onclick="document.getElementById('proof-img').click()" class="w-full py-4 border border-dashed border-gray-800 rounded-2xl text-[10px] text-gray-500 mb-4 uppercase font-bold">Upload Receipt Screenshot</button>
            <button onclick="requestFin('Deposit')" class="w-full bg-[#D4AF37] text-black py-4 rounded-2xl font-black text-xs">CONFIRM DEPOSIT</button>
        </div>

        <div id="wd-modal" class="hidden fixed inset-0 bg-black z-[2000] p-6 pt-12">
            ...
            <button onclick="requestFin('Withdrawal')" class="w-full bg-white text-black py-4 rounded-2xl font-black text-xs">INITIATE PAYOUT</button>
        </div>

        <nav class="nav-bar">
            <div onclick="nav('dash-screen')" class="nav-item active" id="nav-dash">
                <i class="fa-solid fa-house-user text-xl"></i><span>HOME</span>
            </div>
            <div onclick="nav('mine-screen')" class="nav-item" id="nav-mine">
                <i class="fa-solid fa-gem text-xl"></i><span>MINING</span>
            </div>
            <div onclick="nav('office-screen')" class="nav-item" id="nav-office">
                <i class="fa-solid fa-circle-user text-xl"></i><span>OFFICE</span>
            </div>
        </nav>
    </div>
    ...
    <script type="module">
        ...
        window.toggleM = () => { isLogin = !isLogin; document.getElementById('m-text').innerText = isLogin ? "Register New Account" : "Access Terminal"; };
        ...
        window.auth = async () => {
            ...
            if(!id || !pw) return alert("Required Fields Empty");
            const uRef = doc(db, "users", id); const snap = await getDoc(uRef);
            if(!isLogin) {
                if(snap.exists()) return alert("User Registered");
        ...
        function start(id) {
            ...
            onSnapshot(doc(db, "users", id), d => {
                ...
                document.getElementById('v-daily').innerText = "+ $" + (d.data().daily || 0).toFixed(2) + " Daily Profit";
            });
            loadHistory();
        ...
        window.requestFin = async (type) => {
            const amtInput = type === 'Deposit' ? 'dep-amt' : 'wd-amt';
            ...
            if(type === 'Withdrawal') {
                ...
                if(!amt || !addr || snap.data().balance < amt) return alert("Liquidity Error");
        ...
        function loadHistory() {
            onSnapshot(query(collection(db, "logs"), where("user", "==", activeUser)), s => {
                ...
                arr.sort((a,b) => b.time - a.time).forEach(t => {
                    h.innerHTML += `<div class="app-card p-4 flex justify-between items-center text-[10px]">
        ...
        window.nav = (id) => {
            ...
            setTimeout(() => {
                ...
                if(id === 'dash-screen') document.getElementById('nav-dash').classList.add('active');
                if(id === 'mine-screen') document.getElementById('nav-mine').classList.add('active');
                if(id === 'office-screen') document.getElementById('nav-office').classList.add('active');
            }, 200);
        };
